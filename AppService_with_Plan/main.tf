...
resource "azurerm_app_service" "main" {
  ...
  dynamic "storage_account" {
    ...
    content {
      ...
      type         = lookup(storage_account.value, "type", "AzureBlob")
...
#---------------------------------------------------------
# Custom domain and Certificate config - Default is "false"
#----------------------------------------------------------
...
resource "azurerm_app_service_slot" "main" {
  count               = length(var.deployment_slot_name)
  name                = lower(var.deployment_slot_name[count.index])
  app_service_name    = azurerm_app_service.main.name
  location            = local.location
  resource_group_name = local.resource_group_name
  app_service_plan_id = azurerm_app_service_plan.main.id
